...
- `core-transport` matches the Go `SNA1` vector, rejects insecure non-loopback origins, performs strict bounded code-gated registration without following redirects, wraps the returned 32-byte credential with Android Keystore AES-GCM bound to server/workspace/device/HPKE identity metadata, and sends `SNA1` before exposing WebSocket `onOpen` to application code.
- Pixel 10 Pro / Android 16 (API 36) confirms Keystore-wrapped transport credential restoration and silent-replacement refusal in an instrumented runtime test. The transport module remains disconnected from the app and listener until pairing trust and lifecycle integration are ready.
